...
class TkExport:
    ...
    def import_func(root):
        ...
        def enter_excel_dir():
            excel_dir_path = tkinter.filedialog.askdirectory()
            print(excel_dir_path)
            var_excel_dir_path.set(excel_dir_path)

        def enter_xml_dir_path():
            xml_dir_path = tkinter.filedialog.askdirectory()
            print(xml_dir_path)
        ...
        def start_convert():
            ...
            if error is not None:
                if error.isError():
                    tkinter.messagebox.showerror(title='Error', message=error.get_desc(), parent=window)
                else:
                    tkinter.messagebox.showinfo(title='congratulation', message=error.get_desc(), parent=window)
